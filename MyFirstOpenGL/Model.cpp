...
void Model::Render(Camera* myCamera) {
    ...
    glm::vec3 totalRotation = glm::radians(rotation);
    ...
    glm::mat4 view = glm::lookAt(myCamera->position, position, myCamera->localVectorUp);
    glm::mat4 projection = glm::perspective(myCamera->fFov, (float)WINDOW_WIDTH / (float)WINDOW_HEIGHT, myCamera->fNear, myCamera->fFar);
...
/*void Model::UseProgram(Camera* myCamera)
{
    
    GLuint myProgram = ProgramManager::getInstance().compiledPrograms[_programID];
    glUseProgram(myProgram);

    glm::mat4 translationMatrix = ProgramManager::getInstance().GenerateTranslationMatrix(glm::vec3(0.f, 0.f, 1.f));
    //glm::mat4 rotationMatrix = ProgramManager::getInstance().GenerateRotationMatrix(glm::vec3(1.f, 1.f, 1.f), rotation.x);
    //glm::mat4 scaleMatrix = ProgramManager::getInstance().GenerateScaleMatrix(scale);
    //glm::mat4 view = glm::lookAt(myCamera->position, myCamera->position + glm::vec3(0.f, 0.f, -1.f), myCamera->localVectorUp);
    glm::mat4 view = glm::lookAt(glm::vec3(0.f, 1.f, -1.f), glm::vec3(0.f, 1.f, 1.f), myCamera->localVectorUp);
